@echo off
chcp 65001 >nul
title BI Система - Менеджер запуска

:MENU
cls
echo ================================================================================
echo                         BI СИСТЕМА - МЕНЕДЖЕР ЗАПУСКА
echo ================================================================================
echo.
echo   Выберите режим работы:
echo.
echo   1. Интерактивный дашборд (Dash)
echo      - Веб-интерфейс на http://localhost:8050
echo      - Полная интерактивность
echo      - Требует запущенный сервер
echo.
echo   2. Статический HTML отчет
echo      - Создает файл bi_report.html
echo      - Можно открыть в любое время
echo      - Легко отправить по email
echo.
echo   3. Оба варианта (HTML + Дашборд)
echo.
echo   4. Установить/Обновить зависимости
echo.
echo   0. Выход
echo.
echo ================================================================================
echo.

set /p choice="Ваш выбор (0-4): "

if "%choice%"=="1" goto DASHBOARD
if "%choice%"=="2" goto HTML_REPORT
if "%choice%"=="3" goto BOTH
if "%choice%"=="4" goto INSTALL
if "%choice%"=="0" goto EXIT
goto MENU

:INSTALL
echo.
echo ================================================================================
echo 📦 Установка зависимостей...
echo ================================================================================
echo.

python --version >nul 2>&1
if errorlevel 1 (
    echo ❌ Python не найден!
    echo.
    echo 💡 Установите Python с https://www.python.org/downloads/
    echo    При установке обязательно отметьте "Add Python to PATH"
    echo.
    pause
    goto MENU
)

echo ✅ Python найден
echo.
echo 📥 Установка пакетов...
echo.

pip install pandas openpyxl plotly dash dash-bootstrap-components numpy -q

if errorlevel 1 (
    echo.
    echo ❌ Ошибка при установке!
    pause
    goto MENU
)

echo.
echo ✅ Все пакеты установлены!
echo.
pause
goto MENU

:DASHBOARD
echo.
echo ================================================================================
echo 🚀 ЗАПУСК ИНТЕРАКТИВНОГО ДАШБОРДА
echo ================================================================================
echo.

REM Устанавливаем путь к данным
set DATA_FILE_PATH=\\FS\Users\Private\GFD\Public\Трейд-маркетинг\7.Общие документы\Гусев\итог\data.xlsx

REM Если файл в этой же папке
if exist "%~dp0data.xlsx" (
    set DATA_FILE_PATH=%~dp0data.xlsx
)

echo 📂 Путь к данным: %DATA_FILE_PATH%
echo.
echo 📊 Запуск дашборда...
echo 💡 После запуска откройте в браузере: http://localhost:8050
echo 💡 Для остановки нажмите Ctrl+C
echo.
echo ================================================================================
echo.

python "%~dp0bi_dashboard.py"

if errorlevel 1 (
    echo.
    echo ❌ Произошла ошибка!
    echo.
    pause
)
goto MENU

:HTML_REPORT
echo.
echo ================================================================================
echo 📊 ГЕНЕРАЦИЯ HTML ОТЧЕТА
echo ================================================================================
echo.

REM Устанавливаем путь к данным
set DATA_FILE_PATH=\\FS\Users\Private\GFD\Public\Трейд-маркетинг\7.Общие документы\Гусев\итог\data.xlsx

if exist "%~dp0data.xlsx" (
    set DATA_FILE_PATH=%~dp0data.xlsx
)

echo 📂 Путь к данным: %DATA_FILE_PATH%
echo.

python "%~dp0generate_html_report.py"

if errorlevel 1 (
    echo.
    echo ❌ Произошла ошибка!
    pause
    goto MENU
)

echo.
echo ================================================================================
set /p open="Открыть отчет в браузере? (y/n): "
if /i "%open%"=="y" (
    start "" "%~dp0bi_report.html"
    echo ✅ Отчет открыт!
)
echo.
pause
goto MENU

:BOTH
echo.
echo ================================================================================
echo 📊 ГЕНЕРАЦИЯ HTML ОТЧЕТА
echo ================================================================================
echo.

REM Устанавливаем путь к данным
set DATA_FILE_PATH=\\FS\Users\Private\GFD\Public\Трейд-маркетинг\7.Общие документы\Гусев\итог\data.xlsx

if exist "%~dp0data.xlsx" (
    set DATA_FILE_PATH=%~dp0data.xlsx
)

echo 📂 Путь к данным: %DATA_FILE_PATH%
echo.

python "%~dp0generate_html_report.py"

echo.
echo ================================================================================
echo 🚀 ЗАПУСК ИНТЕРАКТИВНОГО ДАШБОРДА
echo ================================================================================
echo.
echo 📊 Запуск дашборда...
echo 💡 После запуска откройте в браузере: http://localhost:8050
echo 💡 Для остановки нажмите Ctrl+C
echo.
echo ================================================================================
echo.

python "%~dp0bi_dashboard.py"

if errorlevel 1 (
    echo.
    echo ❌ Произошла ошибка!
    pause
)
goto MENU

:EXIT
echo.
echo 👋 До свидания!
echo.
timeout /t 2 >nul
exit

